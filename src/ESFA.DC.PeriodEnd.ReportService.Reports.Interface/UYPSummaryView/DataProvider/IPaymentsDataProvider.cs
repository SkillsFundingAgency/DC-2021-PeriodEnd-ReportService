...
namespace ESFA.DC.PeriodEnd.ReportService.Reports.Interface.UYPSummaryView.DataProvider
{
    public interface IPaymentsDataProvider
    {
        ...
        Task<ICollection<DataLock>> GetDASDataLockAsync(int ukprn, int academicYear, CancellationToken cancellationToken);

        Task<ICollection<HBCPInfo>> GetHBCPInfoAsync(int ukprn, int academicYear, CancellationToken cancellationToken);
